...
3. Improved the calculation (toMixedNumber?) or the number normalization to produce simpler output for special cases. For example: 6.4 / 3 currently returns 2_225179981368525/1688849860263936 . The nominal is correct, though there is a better representation: 2_0.4/3 . That (overly) large numerator and denumerator in that example indicates possible performance problem (the greatest-common-denomitator calculation had to run many times)..., and for some cases can even can lead to a crash (max integer threshold exceeded).